...
**OBSERVED live-app seam through the unentitled persistence boundary; ASSUMED signed effects:** the
fourteen `account.connect/macos` evidence entries prove the real core/Darwin wire half and the Swift
presentation half independently. `DulcetMacApp` and its hosted control consume the same
`DulcetMacProduction.makePresentationStore` composition, so the test does not reconstruct the
shipping connector, data source, credential store, or presentation store. The app's production root
view remains compiled but is not instantiated by this state-level control. Hosted run `32598531311`, job
`apple-ci`, first built that app and test host, then observed launchd refuse to spawn the
ad-hoc-signed host while it declared the production `keychain-access-groups` entitlement; no
application or test code ran. The production target keeps that entitlement, while the app-hosted
pull-request invocation clears it so the host can launch without a signing identity, certificate,
provisioning profile, or repository secret.

Hosted run `32599896419`, job `apple-ci`, then launched the unentitled app host, supplied the real
pinned conformance URL, and ran the production composition. Every state assertion before the view
probe passed: the request left `accountConnecting`, reached `accountErrorPersistence`, carried
`credentialPersistenceFailed` in both status and connectivity, retained no active-account pointer,
and loaded no persisted credential. Reaching those guards requires the real server success to cross
`AppleAccountConnectionClient` on the Kotlin main dispatcher into Swift `@MainActor` and enter the
production connected-result persistence branch. The run's only XCTest failures were two later
offscreen-accessibility string lookups; the runner subsequently reported an unexpected app exit and
restart. That accessibility tree is therefore not evidence of what SwiftUI renders in this hosted
environment. The control
`DulcetMacAccountConnectAppTest/connectSuccessCrossesLiveKotlinFacadeIntoPersistenceFailureState`
pins only the observed production state boundary. SwiftUI rendering of that live-derived state
remains compile-checked in the app and independently bounded by the fake-connector presentation and
deterministic capture controls.

The macOS cell remains `partial`: a production Keychain save, its signed-item attributes, and the
...
- **Server-side change** (duration or size changed since download) is detected at the next sync and
  ...
  detail read answers code 70 — §16.11's rules, which hold whether or not the server keeps missing
  files; `getSong` answering `ok` is not evidence the file exists. A `gone` track raises the §16.5
  rule 4 reconciliation; the file still plays. The track's display metadata is pinned for as long as
  the download exists (§16.13).
...
the same (§28 revision 104 item 28). Online, "submit" is a flush, and a change the server refuses is told like any refusal.
...
never merely once `close()` has returned (§28 revision 104 item 28).
...
1. **`getAlbumList2` carries `songCount`**, so an album's track count is drawn without its track
   ...
   songs. `songCount` is a display hint, not a membership count (§16.11).
...
never opened does not, unless it was already on the device when this architecture arrived (§16.17).
The first paint of any screen is at most one request. The full local mirror of §16.2–§16.6 — a
whole-library import into generation-versioned tables — is **not** the target architecture and is
retired by the phases in §16.18.
...
a cost argument. §16.19 records whole-library offline as an explicit, later, opt-in action that
would fill the same cache with those walks — decided in principle, not scheduled.

**Three jobs, separated.** The mirror fused them; the reader keeps them apart.

| job | mechanism | cost |
|---|---|---|
| render the screen being looked at | one live request for the visible page (§16.9) | 1 request to first paint; 1 `getScanStatus` after each page (§16.12) |
| remember what was seen | write-through into the seen-cache (§16.10), plus bounded look-ahead (§16.13) | free for what was shown; bounded for look-ahead |
...
the UI reads, every publication says how old it is (§16.14), and a pending local change the person
made is overlaid on top of both (§16.20).
...
- **Activity-ordered lists are single pages for a related reason:** their order moves with the
  person's own plays and ratings, which the scan stamp never sees (§16.11), so stitching two pages of
  them has no consistency guarantee at all.
...
- **Every value is the one the server returned.** A window stores the server's *positions*, so the
  ...
  (the local views of §16.14), and labels them as such.
...
other rows are unaffected. Opening the screen online costs N requests plus nothing for the epoch if a
foreground reading exists (§16.12). Rows are single pages, so no row is ever stitched. The `random`
row's cached page is shown offline as what it was, never re-shuffled locally and presented as new.

**Measured shape of the live reads.** Unless stated otherwise, every OBSERVED 2026-09-22 measurement
in §16.8–§16.20 was taken against a disposable Navidrome 0.63.2 holding 2,498 albums / 4,996 tracks,
read as a **non-admin** user over loopback, with the conformance fixture's configuration — which sets
**`PurgeMissing = "always"`** (`tools/conformance-env/navidrome.toml.template`). Navidrome's default
keeps missing files instead; a measurement whose outcome depends on that setting names the setting
beside its marker, and those that were run under both say so.
...
OBSERVED 2026-09-11, read-only, on a real library of the same order as §16's target scale on a local
network: 500-album pages took about 0.1 s each and `getScanStatus` 4.3 ms — so one screen-sized page
is tens of milliseconds on a LAN, and the request *count* is what a slow link charges for (§16.2).

`X-Total-Count` is an HTTP header on `getAlbumList2`, never in the body, and absent on `search3`
(§16.6). OBSERVED 2026-09-22 (fixture configuration, `PurgeMissing = "always"`) per list type (this table is the expected presence CONF-75 pins):
...
the catalog payloads carry **per-user state**. OBSERVED 2026-09-22 (fixture configuration, `PurgeMissing = "always"`): after a non-admin user starred,
rated (4) and scrobbled one album — each call returning `ok` — that album's `getAlbumList2` entry and
its `getAlbum` both carried `starred`, `userRating: 4`, `playCount: 1` and `played` for that user,
while the same album read as the admin user carried none of them. A cache keyed by server URL would
show one user's favourites to another.
...
### 16.11 Staleness, invalidation and gone-ness — the catalog epoch

**There is no HTTP-level revalidation to use.** OBSERVED 2026-09-22 (fixture configuration, `PurgeMissing = "always"`) on every `/rest` JSON endpoint
...
bytes. The instrument could see headers — the same `getAlbumList2` responses carried `X-Total-Count`
— so the absence is a finding and not a blind probe. The reader has no per-entity validator from the
protocol and does not invent one: an entity is revalidated by **reading it again**.
...
- **`lastScan` is the scan clock** (§16.1, OBSERVED 2026-09-11): it moved on every real content
  ...
  missed change. It is published **before** `scanning` clears (12 of 12 trials). Its fraction is
  variable-width and its offset is the server's local one: it is compared **as a raw string for
  equality only** — never ordered, never parsed into an instant that "knows" two spellings are equal.
- **The folder-id set** is the one input that answers *as this user*. OBSERVED 2026-09-22 by an
  independent review run (`PurgeMissing` unset): assigning a second library to a user moved the albums that user could list
  from 8 to 9 while `lastScan` stayed unchanged — a catalog change the stamp alone cannot see, and one
  the folder-id set does see. It is compared as a set.
- **Readable by any user.** OBSERVED 2026-09-22 (fixture configuration, `PurgeMissing = "always"`): a non-admin user reads `getScanStatus` with the
  same payload the admin reads, so the epoch does not depend on the account's role.
- **A sentinel is not a stamp.** OBSERVED 2026-09-22, under both settings: while a fresh server performs its first scan,
  `lastScan` reads `0001-01-01T00:00:00Z`. It was seen beside `scanning == true` in this revision's
...
`played`, playlists or favourites. That is why the visible screen is revalidated when online even
under an unchanged epoch.
...
4. **Nothing off screen is revalidated speculatively**, with two bounded exceptions: detail
   ...
   concurrency 1 after an epoch change so that §14.5's server-side-change detection keeps its promise.
5. **Revalidation failure never evicts and never blanks.** The cached content stays, re-labelled with
   the failure kind and its age (§16.14).

**Gone-ness — defined so it holds whatever the server does with missing files.** Whether a server
forgets a removed file or keeps it as "missing" is configuration, and the two answer differently.
OBSERVED 2026-09-22, removing one two-track album's directory, waiting for the watcher's scan, then
restoring it, under **both** settings, with every probe also run before the removal as a control:
...
1. **Membership comes only from a successful detail read.** A `getAlbum` that returns `ok` is the
   album's complete track list *now*. A cached track of that album that is **absent from that list is
   `gone`**. This is the rule that holds under both settings.
2. **Code 70** on a detail read (`getAlbum`, `getArtist`, `getPlaylist`) marks that entity `gone`.
3. **An album whose successful detail read has zero tracks is `gone`**, not an empty album — a
   Subsonic album exists because its songs do (§16.2 rule 3), and under the default setting this is
   exactly how a fully removed album answers.
4. **`getSong` returning `ok` is never evidence of presence**, and a failed `stream` is a playback
   failure (§12.4), never a deletion signal: under the default setting both describe a file that no
   longer exists as if it did, or as a generic server error.
5. A `gone` entity is kept while pinned (§16.13). A `gone` track with a download or a queue entry
   produces the §16.5 rule 4 reconciliation, whose rename caveat now has a second case: under
   `PurgeMissing = "always"`, removing and restoring the same files changes the **track** ids while
   keeping the album id, so a vanished track id may be the same recording under a new id.
6. An entity deleted on the server and never revisited stays in the offline cache until evicted; it
   cannot play unless downloaded, and it disappears from every list the next time that list is read
   live — under both settings, since lists and `search3` drop it either way. That is the declared cost
   of not enumerating (§16.19).
...
ever held?* — at two granularities.
...
never publishes part of it. A single page is therefore always presentable on its own, which is why the
first page of any list paints after **one** request.

**A window is a contiguous run of pages presented as one list, all read under one epoch** — the
*window epoch* stored on `cache_list`. It may start at any page, not only at offset 0. Offset paging is
...
1. **the check fires** on a page being appended;
2. **the stored window epoch differs from the current epoch** at the window's first live read in this
   session — which covers the stale-while-revalidate case: a window cached last week, reopened, its
   visible pages revalidated and then extended by `loadMore` would otherwise stitch a page read under
   today's epoch to pages read under last week's;
3. **the folder-id set** read at window open differs from the window's.
...
**Scroll position across a rebase** is kept by item id: the first visible item stays first. If that
...
- pages **append**, but each page read while `scanning == true` is marked *unguarded*, and the
  window's coverage becomes `unverified(scanning)`;
- the list is labelled **online** as well as offline: "Your server is updating its library — this
  list may change";
...
100), and is `complete` only when it starts at offset 0 and an **empty** page arrives, or its row
count equals the window's `X-Total-Count`. A short page is a candidate end, confirmed by one more
request when there is no total (§16.2 rule 1). A page that contributes no new id does not end the
window (§16.2 rule 2). The window never loads more than one page beyond the viewport.
...
- **Detection.** OBSERVED 2026-09-22 (`PurgeMissing = "always"`), `alphabeticalByName`, pages of 100:
  with no mutation, eight pages returned 800 distinct albums and a `getScanStatus` after every page
  never differed from the opening reading. With the album at alphabetical position 49 removed after
  the first five pages (0–4) and the watcher's scan allowed to **finish**, the eight pages again
  returned 800 distinct ids — **one album that existed before and after the removal was returned by no
  page**, the silent skip of §16.1 — and the reading differed on page 5, the first page read after
  the removal, and on every page after it. That probe printed its results and asserted nothing, and
  because it waited for the scan to finish it did not exercise a scan racing a page read; CONF-70
  turns it into assertions.
- **The race.** Probed separately by `tools/probes/window-epoch-race`, which reads one page and then
  `getScanStatus` in a loop while a thread moves one album directory in and out of the music folder
...
> so, and a server that reports no scan stamp is stated once for its account. A page is never stitched
> to a page read under a different stamp. Cached content is published with its age and never as live;
> an error never replaces cached content; a slower answer never overwrites a newer one; a pending local
> change is shown over both. Metadata of anything downloaded, queued or playing is pinned.*
...
- **A server with no epoch** (a **successful** reading with no `lastScan`, or only the sentinel — never a
  ...
  occasionally miss an item while the library is changing."
- **Activity-ordered lists** are single pages precisely because no check covers them (§16.9).

### 16.13 What "seen" means, look-ahead, eviction, bounds and pins
...
1. every list page a screen requested and received, plus at most **one page of look-ahead** past the
   viewport;
2. every album, artist and playlist the person **opened** (the detail read, with tracks or entries);
3. **detail look-ahead** (below): the `getAlbum` of albums in or next to the viewport;
4. every entity returned by a search the person ran (the entities, not the result list);
5. the metadata of every track that was queued or played — already cached by (1)–(4), because a
   track can only be queued from a screen that read it — **pinned** while it is in the queue;
6. artwork the UI actually requested, in the artwork cache of §18.2;
7. on upgrade, the rows the retired mirror already held (§16.17).

Nothing else is fetched on the person's behalf: no whole-library walk and no background refresh of
lists nobody is looking at.

**Detail look-ahead**, so that the first tap on an album opens instantly. When a grid or list of
albums has been still for **300 ms**, the reader fetches `getAlbum` for the albums in the viewport and
the next viewport-height beyond it, most-central first, subject to all of:
...
At the sizes measured in §16.9 (~3 KB and ~5 ms per `getAlbum` on a LAN — a two-track album; real
albums are larger, ASSUMED under 20 KB), a settled viewport costs under 0.5 MB and is bounded by
count, not by scrolling speed: a fling that never settles prefetches nothing. (ASSUMED values; tune
from measurement.)

**Pins.** A pinned entity is never evicted and never purged by a revalidation that no longer lists it.

| pin | set when | released when |
|---|---|---|
| `download` | a download row is created — in the **same transaction** — for the track, its album (with credits) and the album's artwork at the 256 px bucket | the download row is deleted |
...
**Bounds and eviction.** Metadata is small next to artwork: at ~617 bytes of JSON per album summary
(§16.9), less once normalized (ASSUMED), even a person who browses an entire 30,000-track library
holds tens of megabytes. So the bound exists to make growth finite, not to ration browsing:
...
### 16.14 Offline, the presentation states, and reconnect
...
and offline. `unverified(noEpoch)` is stated once per account, not per list.
...
| a list never read (a sort order never chosen, a genre never opened) | the **local view** of the cached entities of that kind, sorted locally and labelled "Available offline"; `unavailable` only when there are none |
| an album that was opened or looked ahead | its tracks, each with playability |
| an album seen only in a grid | its cached summary (title, artist, year, artwork if cached), and `unavailable` for the track list — distinguished from an album with no tracks by `detail_complete`, never by `songCount` (§16.11) |
...
loading state is published**; a loading indicator may appear only for the part that has nothing
cached. So an album seen only in a grid opens online with its cached summary header painted at once,
and only its track list shows loading until `getAlbum` lands. The acceptance tests assert the whole
publication sequence, not just its last element, because a spinner frame between two correct states
passes every test that only checks the end.

**Reconnect**, in order, when reachability returns or the app returns to the foreground online:

1. flush the scrobble and mutation outboxes (§15.3, §16.20) — user-authored data first. A flush
   reads no epoch before its writes (§18.6), and one outbox whose flush fails — its own database
   included — is reported and never skips the next outbox or step 2;
...
**As implemented — one reachability model (R2a review, §28 revision 104 item 25).** A reconnect is the
**only** way back online. Steps 1 and 2 run while the reader is still offline — every screen keeps
saying so and nothing reads the server — and only a successful epoch reading marks it online and goes
on to step 3. A platform report that the server is unreachable takes the reader offline at once and
cancels a reconnect in flight; a report that it is reachable, while offline, *requests* a reconnect
rather than marking the reader online. A reconnect already running is joined. So a shell reports
reachability on every change and reconnects on foreground, in either order or only one of them, and
nothing is read before the flush. A reconnect whose epoch reading fails stops there — the flush of
step 1 may already have sent changes, but the reader stays offline and nothing is revalidated or
relabelled — says which failure, and the next report or reconnect tries again. The flush of step 1 sends while the server is
reported reachable, which is what lets it run before the reader is online.
...
**Revalidated like a window (R2a review, §28 revision 104 item 26).** Open searches are part of the
...
- §16.5 rule 4 (deletion reconciliation, with its rename caveat, extended in §16.11 rule 5) and rule 6
  (bounded concurrency).
- The parsers for album summaries, albums, artists and credits in `LibraryBrowse.kt`, and the
  `KtorLibraryEndpointTransport`.
- The `getScanStatus` reading and its comparison rules, which move to the catalog epoch (§16.11).
- Downloads, queue, scrobble and resume tables — untouched except for the pins (§16.13).

### 16.17 Migration of on-device databases
...
1. **Additive (the next schema version, 6 on the current main).** Creates the `cache_*` tables beside
   ...
   - **no list windows are created**: the mirror has no server order to rebuild one from, so offline
     ...
     read live once;
   - **per-user fields are unknown, never false**: `userRating`, `playCount` and `played` are left
     null. `starred` is seeded as *starred, time unknown* for exactly the items in the generation's
     `library_starred` — the same per-user `getStarred2` answer the cache field records — and left null
     for every other item;
   - for every `download` row and every `queue_entry`, the track, its album and their credits are
     then **pinned**. A download or queue entry whose track is absent from the committed generation
     gets a pinned identity-only row marked *metadata missing*, refilled by its next live read — never
     a dropped download.
2. **Subtractive (the version after).** Drops the mirror tables and `committed_generation` once
   R2–R4 have moved every reader off them.
...
`mutation_outbox` and `resume_position`. The additive step additionally asserts a pin for every
download and queue entry and the seeded rows' shape above (CONF-81).
...
- the `lastScan` rules — raw-string equality, "cannot read" is never "unchanged", the folder-id set
  as a second input — become the catalog epoch of §16.11;
...
### 16.18 Platform shells, the facade, and the phased plan
...
facade exposes `subscribeLibraryWindow(request, listener) -> AppleLibraryWindowSubscription`, where the
subscription carries `loadMore()`, `setViewport` and `close()`, per §7.2's event-stream rule. This also
retires `DulcetKit`'s `browse` completion, which is invoked twice for one open (a `preview`, then
`loaded`) — a completion that means "more may follow" is a stream in disguise. Specifics:

- Each publication is a **final, immutable Kotlin class of primitives, `String`s and `List`s of such
  classes**; freshness, coverage and playability are closed string kinds plus nullable epoch-millis
  and counts, as the existing facade already encodes errors (`kind: String`) — no sealed hierarchy
  with a generic payload crosses. No SQLDelight entity, no `Flow`, no generic repository crosses.
...
- The presentation protocols `DulcetLibraryBrowsing`, `DulcetCommittedLibraryBrowsing` and
  ...
  `DulcetLibraryBrowseOutcome.preview`/`.loaded` are replaced by publications with freshness. §16.7's
  "a preview is not an ending" becomes unnecessary: nothing about a publication means "the open
...
control dies on the platform that cannot draw the control; six shells over one core is exactly the
shape that bug aims at.

**The plan.** Each phase is a set of briefs in the §19.4 format. Briefs in the same phase run in
parallel; **each brief owns its files exclusively** — two briefs never edit one file. Every phase ends
with an independent adversarial review that checks commit messages and comments against the code.
...
| **R1d** | mutation outbox and overlay | new `MutationOutbox.kt` (the table exists in `ProtectedReservedData.sq`; no production code does yet), the `mutation_outbox` queries in `ProtectedReservedData.sq` — after R1a | CONF-84 (overlay, compaction, adopt-on-echo), §18.3 conflict rule |
| **R2a** | Apple shared layer | new `core/src/appleMain/.../AppleLibraryReaderFacade.kt`; `DulcetKit/.../AccountConnectionPresentation.swift`, `PresentationModels.swift`, `DulcetLibraryViews.swift`, `DulcetStrings.swift`; `DulcetAppleShared/DulcetAppleProduction.swift` — after R1b–R1d | Swift copy tests per type; header diff reviewed; presentation tests for every freshness, coverage and playability value |
| **R2b** | Apple destinations: macOS; iOS + iPadOS; tvOS (one brief each) | only that destination's view code under `apple/DulcetMac`, `apple/DulcetiOS`, `apple/DulcetTV`, and its app-host tests — after R2a | CONF-76, 77, 79, 84, 86 per destination, each with its own test identity (an iPhone run does not evidence iPad) |
| **R3** | Android phone + TV | `core/src/androidMain/.../AndroidLibraryDatabase.kt` → `AndroidLibraryReader.kt`; `android/shared/.../library/LibrarySession.kt`, `LibraryEntry.kt`; `search/AndroidLibrarySearchSource.kt`, `SearchPresentation.kt` — after R1b–R1d | CONF-76, 77, 79, 84, 86 on `android` and `androidtv` |
| **R4** | downloads and pins | `DownloadPolicy.kt`, `Downloads.sq`, `AppleDownloadFacade.kt`, `DulcetAppleShared/DulcetCoreDownloads.swift`, `PlaybackQueueController.kt` — parallel with R2/R3 | CONF-85: enqueue pins metadata in the same transaction; downloaded albums re-read after an epoch change; a pinned track that goes `gone` (CONF-83) raises reconciliation |
| **R5** | retirement | delete `LibrarySync.kt`, `LibrarySyncControlDatabase*.kt`, `AppleLibrarySyncFacade.kt`; `Library.sq` (mirror tables), `SchemaMeta.sq` (`committed_generation`); a subtractive migration under the next free number (`migrations/<n>.sqm`, `databases/<n+1>.db`, `tools/migration-fixtures/v<n+1>/`); `FEATURES.yml`; `tools/parity_gate.py` and its mutation controls — after R2–R4 | CONF-31..33 retired from the registry; `accepted_removals` implemented with a control proving an undeclared removal still fails; `library.sync` retired through it; subtractive migration preserves protected data (CONF-81) |
...
with a mutation control showing that an undeclared removal still fails and a declared one passes, and
retires `library.sync` through it. The gate change lands in the same phase as the removal, not before,
...
|---|---|---|
| whole-library offline browse | offline shows what was seen (plus what the mirror held at upgrade) | coverage is stated above the list with have/total; local views label themselves (§16.14) |
| complete local search | offline search finds only what was seen | the scope label says so, with counts (§16.15) |
| eager deletion detection | an unrevisited deleted album stays in the offline cache | lists drop it on their next live read; detail reads apply §16.11's gone rules; downloads are re-checked after an epoch change |
| client-side sort over the whole library | sort orders are the server's fixed `type=` set online | offline local views sort locally, labelled local |
| a snapshot of the whole catalogue | consistency is per page and per window, not per library | the post-page epoch check and the stored-epoch rule make a torn window detectable (§16.12) |

**Whole-library offline — decided as an explicit, later opt-in; unscheduled.** A user-initiated
"Make my whole library available offline" action can fill the seen-cache with the §16.2 walks — 48
requests at 2,500 albums — marking everything seen, with no generation machinery, because the
cache's unit of consistency is the response. It is not part of v1 and not on any phase of §16.18; it
is recorded so that nobody re-derives it and nobody mistakes the reader for a decision against it.

### 16.20 Favourites and ratings in a reader — the local change is shown at once

A star or a rating must change on screen in the same frame as the tap, and must not flicker back when
a revalidation lands before the change reaches the server. With one read path (§16.8) that is a rule
about publication, not about the cache:

- A tap writes the mutation to `mutation_outbox` (§18.3: compacted per `(target_id, field)`) and the
  core republishes every open window that contains the target — **the change is in the next
  publication, before any request is sent**.
- **Every publication overlays pending mutations.** For each entity in it, the latest pending
  `mutation_outbox` value for each field replaces the cached value at publish time. The cache row keeps
  what the server last said; the overlay is not written into it. A live read that lands while the
  mutation is pending updates the cache row and republishes, and the overlay still wins — so the star
  never disappears.
...
- **Conflict:** on reconnect the server's value wins **unless** a local mutation is newer than the last
  ...
  **Corrected in R1d (§28, revision 104 item 18):** read literally, a live read issued after the
  ...
  was set after it.
  **Which changes a later read can reach (R2a review, §28 revision 104 item 25).** Only changes made
  while the reader is connected: one whose send failed, or one waiting behind another send. A change
  made offline cannot be overtaken, because a reconnect flushes it before anything is read (§16.14
  step 1): it is sent, and it replaces a value another client set while this device was offline.
  That is §16.14's order — user-authored data first — not a new rule; the first reader API let a read
  run before the flush and so sometimes decided otherwise.
...
| `formPost` | advertised; the repeated parameters above work as a form body — a 2,400-entry replace, duplicates in a scrambled order, kept exactly |
| `getPlaylists` | each playlist carries `owner` and a `created` time with a UTC offset (for example `…T22:48:25.888372-04:00`) |
...
3. **Sending re-reads the playlist first** (`getPlaylist`, written through to the cache), and holds
   **one** of the server's request slots (§16.14's concurrency bound) from that read through the
   write, so no other request of this session queues between them. Then:
   ...
   - the change only **removes** → `updatePlaylist?songIndexToRemove=` with the positions of the
     removed entries **in the list just re-read**, highest first, so no removal shifts another and a
     server applying them one by one removes what one applying them to the list as found removes.
     This includes emptying a playlist, which a replace cannot do;
...
**The exposure that remains, stated honestly.** Subsonic has no conditional write, so a change
...
scrobbles (§16.14 step 1). A flush reads no catalog epoch before its writes: its reads are written
through under the session's reading, and step 3 of §16.14 revalidates them. A flush whose own
database fails stops, keeps every change as it was, and reports it; it never throws into the
reconnect, which goes on to the next outbox and to reading the epoch.

**At least once.** A write is marked attempted before it is sent, with the list the server holds if
it lands (`sent`) — for an append, the list it was sent onto plus the songs it appends. A lost answer
leaves the change pending, and the re-read before the retry shows whether it landed: the server holds
`target`; or `sent`; or `sent` less song ids this device added that the server does not know (it drops
those silently, OBSERVED) — this device's own write, even after a later edit changed `target`. A
...
again on a guess** (maintainer's decisions, second to fourth review rounds). Before each send of a
create the device lists the server's playlists (`getPlaylists`, one request) and records with the
outbox row the ids of **every** playlist listed, whatever its name (`seenBeforeSend`): none of those
can be what the send makes, even one renamed into the name later. When another create of this
device is settled — its id answered, adopted or chosen — that id joins the record of every create
still in doubt; and every id offered to the person for a create — `PossiblyCreated` or
`PossibleDuplicate` — joins the record of every other create of this device the moment it is
offered, whatever their names (fifth review round). So a playlist this device knows one of its
creates made, or has offered to the person for one, is never a candidate for another, after a
relaunch too; an id mapped to another create in this session is excluded as well. A create waiting
on a choice that named an id another create then settles loses that id from the choice: a choice the
person made of it is void, and the create is asked again with the candidates that remain
(`PossibleDuplicate`) — never adopting one the person passed over — or, with none left, is looked
at again (only a server's own answer naming a playlist already offered, or a row persisted before
this rule, reaches that case). After a create whose answer was lost — or that the server answered
with an empty `ok`, naming nothing — the playlists are listed again, and the **candidates** are those
with the name that send carried and **not** in the record, **whoever the server says owns them**. An
owner decides only adoption (below), never whether a playlist is a candidate: the owner is compared
ignoring case, since the reference server signs a login in whatever its case and states the account
as it was created (`u=admin` is owner "Admin", `u=DULCET-ADMIN` is owner "dulcet-admin", both OBSERVED
2026-09-24, the first by the review; CONF-88..91 pass signed in with the second spelling), and a
server may state this account in another form altogether — a display name — which no comparison
recognises (ASSUMED: no such server was observed). **No clock enters it** — not the server's `created` stamp, its timezone, or
...
- **No candidate** — nothing is listed that the send could have made: it is sent again. This is not
  proof that the send did not land: a server that commits a timed-out create only after the next
  flush has listed its playlists ends with two, and nobody is told (residual 4 below). After an
  empty `ok` that made nothing visible, it is also this change's failure, counted toward the three
  of "Failures".
- **Exactly one candidate, owned by this account or with no owner stated, that holds the songs
  sent, while no other create of this device that sent the same name is in doubt,** is adopted.
  Another create of that name in doubt — waiting for the person, deleted here, or not yet looked at
  — may have made that very playlist, so while one is, a lone candidate is named, never adopted
  (fifth review round; a row that never recorded the name it sent counts as the same name). "Holds
  the songs sent" admits exactly this: the songs sent, in the order sent,
  or that list with some songs left out — every copy of each — but never all of them; any
  difference from the songs sent is then told (`Diverged`). The device cannot tell a song id the
  server dropped because it does not know it from a playlist another client made with only some of
  the songs, so both are adopted and told. A wrong adoption can only merge the create into a
  playlist of that name, this account's, holding some of those songs in order, made after the send
  (residual 1 and 3 below). A single candidate holding other songs, or the same songs in another
  order, is not adopted: this is stricter than the decision's "one candidate: adopt it", because
  adopting a namesake another client made with other songs would lose this create.
- **Otherwise** — several candidates; one while another create of that name is in doubt; one
  holding other songs; or one whose stated owner is not this account — **nothing is sent again
  automatically.** An owner that does not match makes a
...
- A create **deleted here** while its send was in doubt deletes **nothing** — whether it was deleted
  ...
  already out when the delete arrives is not recalled and may land (fifth review round). Only an id
  the server's answer named is deleted without the person. The record holds every playlist that existed before the
  send, so an old "New Playlist" is never offered, whatever its `created` says, whether or not it
  can be read, and whatever it was called when the send went.
...
unless it holds the songs sent; (4) a server that commits a timed-out create only after the next
flush has listed its playlists sees it sent again, and ends with two playlists nobody is told of. The
same holds for a candidate whose stated owner is not this account: it is named to the person, never
adopted, so a create on a server that states this account in another form always asks. (5) With two
creates of one name in doubt, what either made is offered to the person for the other rather than
adopted, and a create whose own playlist was offered for another is sent again — a duplicate the
person can delete, never a playlist lost or merged into another create's. ASSUMED: one
`getPlaylists` lists every playlist of this account in one response (the endpoint takes no paging
parameter; OBSERVED by the review, 600 playlists in one response on the reference server).
...
- **held** — the flush stops, every change is kept, nothing counts toward a drop, and the person is
  ...
  - **a rate limit (429).** The flushes wait `max(Retry-After, floor)`. The floor is 2 s and doubles
    ...
    is one run, told once, its floor doubling throughout (fourth review round: ending the run on
...
- **stopped** — no answer, or a gateway that cannot reach the server: 502–504, and the origin
  errors a CDN in front of the server answers with, 520–524 and 530 (each listed on Cloudflare's
  5xx reference, OBSERVED 2026-09-24): every change kept, nothing counted. Like any 5xx they prove
  nothing about whether the request was applied, so a create among them stays in doubt and is
  settled by its pre-send record;
...
what it may have made (`AlreadySent`, then `PossiblyCreated`).
...
- appends — including a create's songs beyond what its own request carries — go in **batches**, in
  order, each recorded once it is on the server so a later failure never sends it again;
- removals go in batches, highest positions first, each batch verified by the read that follows the
  one before it;
- a whole list that does not fit is **refused with nothing written**, and the person is told why
  (`CapabilityUnsupported(PlaylistWholeListWrite)`): chunking a replace is impossible without a window
...
fresh disposable server before landing; items 11–14 are what that review changed.

1. **The decision and the argument against it (§16.8, §16.19).** The server is already the indexed
   database; Dulcet is single-provider; its offline promise is "what I browsed". Revision 98's finding
   that the mirror costs only 48 requests at 2,500 albums is kept as the strongest counter-argument.
   Whole-library offline is decided as an explicit, later, user-initiated opt-in and is unscheduled.
2. **One read path (§16.8).** Live responses are written through into the seen-cache and every screen
   is published from it, online and offline, with a freshness value — `live`, `cached(asOf?, reason)`
   or `unavailable(reason)` — and with pending local changes overlaid (§16.20).
3. **The data model (§16.9–§16.10).** Every screen maps to one request shape; a home screen is N
   independent single-page windows. The cache is keyed by provider instance and canonical request,
   with normalized entities, server-ordered windows, pins, writes ordered by request issue time, and a
   structural binding to the server and user it was filled from. OBSERVED 2026-09-22: catalog
   payloads carry per-user state for the reading user only; `X-Total-Count` presence per list type is
   tabled; `getArtists` varies its `roles` order between calls, so responses are compared by parsed
   value, never by bytes.
4. **Staleness without validators (§16.11).** OBSERVED 2026-09-22: `/rest` JSON reads carry no
   ...
   covers the catalog and never user state. OBSERVED: `getScanStatus` is readable by a non-admin
   user; the first-scan sentinel `0001-01-01T00:00:00Z` is "no epoch" whatever `scanning` says beside
   it; and, by an independent review run, a library assignment moved a user's albums 8→9 under an
   unchanged `lastScan`, which the folder-id set catches.
5. **CORPUS §4 line 11 replaced (§16.12).** A reader has no committed generation to pin. A page is
   one server read; a window is a contiguous run of pages under one stored epoch, extended only when a
   `getScanStatus` read *after* each page shows that epoch unchanged and no scan running. A window is
   torn by a fired check, by a stored epoch that differs from the current one at its first live read,
   or by a changed folder set, and is then rebased around the viewport only. While the server scans,
   pages append marked `unverified(scanning)` and say so online. OBSERVED 2026-09-22: a scanned
   deletion after five pages skipped one album and changed the reading from the sixth page on; and
...
6. **Gone-ness that holds whatever the server does with missing files (§16.11).** OBSERVED
   ...
   track ids; with the server's default, `getAlbum` answers `ok` with `songCount: 2` and zero songs,
   `getSong` answers `ok`, and `stream` returns a generic code-0 envelope. Both drop the album from
   lists and `search3`. So membership comes only from a successful detail read, and neither
   `getSong` nor `songCount` is evidence of presence — §16.7 property 1 and §14.5 are corrected.
7. **Offline, look-ahead and reconnect (§16.13–§16.14).** "Seen" is defined item by item, including
   bounded detail look-ahead (≤24 per settled viewport, ≤2 in flight, never on a constrained
   network). Eviction is least-recently-*accessed* under row-count ceilings; downloaded, queued and
   playing items' metadata is pinned, because the `download` table holds none. No spinner frame may
   precede a cached paint, online or offline. Reconnect is outbox flush, epoch read, visible-screen
   revalidation, downloaded-album recheck, and nothing else.
8. **Search scope (§16.15, §18.1)** — every result names its source, with the cache's counts offline.
9. **Favourites and ratings (§16.20)** show in the same publication as the tap and survive a
   revalidation that lands before the send: pending outbox values are overlaid at publish time.
10. **Retirement and migration (§16.16–§16.17).** `LibrarySync.kt`, the generation tables, the sync
    facades and CONF-31..33 are deleted in phase R5. The additive migration seeds the seen-cache from
    a `verified` committed generation — unpinned, stale, age unknown, per-user fields unknown,
    `starred` only from `library_starred`, no windows — and pins every download and queue entry, so
    offline works on the first launch after upgrading; a subtractive migration later drops the mirror.
    The unmerged instant-open change is folded in, not shipped.
11. **A gate change, decided.** `tools/parity_gate.py` fails unconditionally on a removed
    `FEATURES.yml` row. R5 adds an `accepted_removals` declaration with a control proving an
    undeclared removal still fails, and retires `library.sync` through it.
12. **Review corrections to the first draft of this revision.** It said code 70 marks a removed album
    — true only under `PurgeMissing = "always"`. It defined a torn window only during paging, so a
    week-old cached window revalidated and then extended would have been stitched across two epochs.
    It refused to append while scanning, which freezes scrolling for a whole scan. It claimed a
    one-request first paint while spending three. Its tear re-read was the whole scroll depth. It
    stated a migration age the database does not record. And it described a print-only probe as if
    it asserted, and as if it had exercised the mid-page race it never did. Each is fixed above.
13. **Corrections in place.** §16.1: `getIndexes?ifModifiedSince` is CONF-34, not CONF-31, and its
    claimed blindness to a changed track under an unchanged artist was false of the detection. §9.3
    `libraryChangeSource`, §11.3–§11.5, §14.1, §14.5–§14.7, §16.7 property 1, §18.1–§18.3 and the
    §20.4 and `docs/CONFORMANCE.md` registries (CONF-70..87 added: 70–75 server facts, 76–87
...
21. **Playlists in the reader (W9, core).**
    ...
    sequence, then reject) did not say how an accepted edit is sent. The editor never sends a
    position computed from the person's view: an entry edit names the view it was made on, compacts
    to `(base, target)`, and re-reads before sending. **Corrected by the review round below** (the
    first cut wrote every positional edit as a whole list, and called its residual exposure one that
    the read-back "cannot always see"): a removal sends the positions of the chosen entries in the
    list just re-read, highest first, and is kept only if exactly those entries went missing; an
    append stays an append; a move or insert writes the whole list by `createPlaylist?playlistId=`,
    which OBSERVED keeps the playlist; every write is read back. The residual exposure of each is
    stated in §18.6 — and for a whole-list write, **an overwrite by a whole-list replace cannot be
    detected in the window**. **Offline playlist editing reverses from "disabled" to queued** (§16.14,
    ...
    `playlist.<kind>` (schema unchanged). **Schema 7 adds `cache_playlist.comment`, `is_public` and
    `readonly`** by the additive `migrations/6.sqm`, appended last, with `databases/7.db` and a v7
    fixture holding cached playlists; the migration gate passes (fifth review round below). The reader gained named hooks — `LibraryPlaylistOverlay`, `republishPlaylists`,
    ...
    **Item 21, review round — corrections after an independent review of the first cut.**
    ...
    whole-list write, whose overwrite in the window is never detectable. (d) The re-read and the
    write now hold one request slot between them. (e) An append records the list it is sent onto
    (`sent`), so an append in doubt is recognised even after a later edit, and every in-doubt check
    allows for song ids the server dropped. (f) `formPost` is a required session parameter; an HTTP
    error status without an envelope is `Server.HttpStatus`, where it was a malformed envelope
    (413/414 refused, 502–504 stop the flush, only a 4xx proves the change was not applied, for
    favourites as for playlists); without `formPost`, appends and removals are batched within
    7,000 bytes of parameters (ASSUMED 8 KiB proxy limit) and a whole list that does not fit is
    refused with nothing written — OBSERVED against the reference server, which also keeps a
    2,400-entry form-body replace in order (CONF-88). (g) A playlist flush whose own database fails
    reports it and no longer throws into reconnect, which skipped the scrobble flush and the epoch
    read; a flush reads no epoch before its writes (§16.14 step 1). Found while fixing (a): a
    rename or song change made while a create was in doubt made the proof look for the edited
    playlist, so the create was sent again; the name and songs a send carried are now recorded
    with it, and the edits follow the adopted playlist. Found by this round's mutation run: a write
    in doubt that never arrived was read as "landed, every new id dropped" — an append was never
    sent again and an insert of new songs was reported diverged — and a positional edit over an
    append in doubt lost the record of its songs, so they could be appended twice; each has a
    failing-first test. (h) **The first cut's commit
    message claimed "18 of 18 applicable mutants killed". That was false:** one of the 18 (skip the
    base verification) did not compile and was counted as killed, and its compiling equivalent
    survived the review. This round's run records compile status and never counts a mutant that did
    not compile. On the final commit: **76 mutants** — the first cut's 19 adapted, the reviewer's
    12 plus 2 variants, and 43 new — **74 killed, 2 survived, 0 failed to compile**, with a control
    mutant that must not compile reported as not compiling and not counted. The two survivors are
    stated, not hidden: the failure guard around the playlist flush inside the reconnect composition
    is equivalent (that flush reports its own failures and never throws), and the check that an
    append in doubt still heads the songs to add is reachable only through the ASSUMED ends-with
    ...
    **Maintainer's decision:** a refusal of access holds only when an authenticated `ping` is refused
    ...
    five minutes, a delivered change ending the row, the person told once per row. **Superseded by
    S6 of the fourth round below:** a run ends when a flush sends something and meets no 429, or
    when the queue empties — never on one delivery — and, from the fifth round, whenever a flush
    finishes with nothing pending; a 429 for a change no longer queued begins none. The parser reads a
    ...
    send the device records which playlists of that name the server already lists (**superseded by
    S4 of the fourth round below:** it records every playlist listed, whatever its name, and from the
    fifth round every id offered to the person for another create joins that record); after a lost or
    ...
    when a new 429 arrives. **Corrected by the fourth round below:** the argument given here — "a
    429 can only come from a flush that ran after the previous wait ended" — was false: a flush that
    started before the wait was set can meet one (r6), and that is how a second 429 shortened a
    longer wait. The mutant was re-run against the fourth round's code, and its verdict and reason
    are in (z) — **a verdict the fifth round withdraws: the mutant is killed there (ad).** Re-run on the final code: 49 mutants — the survivors, the
    ...
    **Item 21, fourth review round — a re-review of the third round's head found no blocker; the
    ...
    creates, one playlist, only `Created` told (r2a); applied before its 500, the first was offered
    the second's own playlist (r2b). **Decision:** an id mapped to another create here is never a
    candidate, and each id settled — answered, adopted or chosen — joins the pre-send record of every
    create still in doubt, so the rule holds across a relaunch; a create already waiting on a choice
    that named it is looked at again without it, and settles if that leaves one candidate that holds
    its songs. **Corrected by the fifth round below:** "one create's playlist is never a candidate
    for another" did not hold for a playlist offered to the person for another create, nor for what
    another create still in doubt may have made (ab); and a choice another create voids is asked
    again, never settled silently (ac). **S2: a delete made while the flush
    looked for a lost create deleted by inference.** The person deleted the create while its lone
    candidate was being read; it was adopted, then deleted by id (r1, on the JVM and on macOS),
    against "nothing is deleted on inference". **Decision:** delivery knows whether an id was the
    server's answer or inferred — adopted, chosen, or left by an empty `ok` — and an inferred one
    deleted here meanwhile ends as `PossiblyCreated([id])`, with nothing written to it, the create's
    comment and visibility included (**narrowed by the fifth round below:** a write already out when
    the delete arrives may land, (ae)). **S3: an owner alone ruled a candidate out.** A server stating the
    owner in another form than the sign-in name excluded the create's own playlist, so it was sent
    again beside it (r11). **Decision:** an owner never rules a candidate out; one that does not match
    is named (`PossibleDuplicate`), never adopted. The second round's test that another owner's
    namesake is never named is inverted. **S4: only namesakes were recorded**, so an old playlist
    renamed into the name while a create was in doubt became its candidate: offered for deletion
    (r4a) or adopted by an empty create (r4b). **Decision:** the record holds every listed id, from
    the listing already fetched. **S5:** "the send provably did not land" was false — a server that
    commits a timed-out create after the next listing ends with two (r3). The text now states that
    residual; the zero-candidate rule stands, and r3 is pinned as residual 4 of §18.6. **S6: a run of
    429s ended on every delivery**, so under a limiter admitting one request per window eight
    favourites took fifteen requests and told `Held` seven times, and the floor never grew (r5).
    **Decision:** a run ends when a flush sends something and meets no 429, or when the queue
    empties; each outbox has its own run and tells it once; the floor doubles through it. **S7: a new
    429 shortened a longer wait.** Favourites met `Retry-After: 60`, the playlist flush in flight met
    a 429 with none, and the wait became 4 s — the star was sent again well inside the server's 60 s,
    and the playlist change's `Held` was never told (r6). **Decision:** the later end stands; each
    outbox is told on its own run. **S8:** two withdraw mutants survived the suite (a withdrawn
    favourite or rename still shown); r7 and r7b are now tests. **NITs:** a change withdrawn after a
    send of it went out and was not answered with a 429, another 4xx or another answer proving it
    unapplied answers `AlreadySent`, not `CompactedAway`, in both outboxes, and the display follows
    the server's answer (r12; **worded so by the fifth round below** — this text said any send that
    went out, which the code never did: a send so answered answers `CompactedAway`); "holds the songs sent" now says what it admits — the
    songs sent in order, or some of them, adopted and the difference told (r10a); the origin errors
    a CDN answers with, 520–524 and 530, join 502–504 as a stop that counts nothing; CONF-89 names
    its playlists afresh on every run and cleans up the two it did not register, so a server reused
    after a failed run measures the same. (y) **Failing first.** Run against the third round's head before any
    fix, the fourth round's tests — the reviewer's probes adopted where the decisions say pin, and
    one test for each further edge of a rule — failed 24 of 37 on the JVM and the same 24 on macOS,
    r1 among them on both, each on an assertion naming the defect. The 13 that passed there pin
    behaviour that already held: r3 (the stated residual), r7, r7b, r8, r9a, r9b, r10a with its
    out-of-order control, r10b, a delete of a create whose answer named its id, a run ending once a
    flush meets no 429, a withdrawal of a change never sent, and a flush the wait stopped leaving
    its run alone; each earns its place by killing a mutant below. Three earlier tests changed by
    decision: another owner's namesake is named (S3), and withdrawing a create in doubt answers
    `AlreadySent` (r12). All 37 pass on the fourth round's code on both. (z) **Mutation run**,
    compile-gated, on the fourth round's commit in a worktree of its own, each mutant restored by
    `git checkout` and the tree verified clean: 39 mutants — at least one for each decision
    above — and a control that does not compile, reported so and not counted. 37 were killed.
    Two survived. One had no test: a run of 429s whose last change a read shows already applied,
    so the flush that drops it sends nothing and the run ends only because the queue is empty;
    it now has one, shown failing under its mutant and passing on the code. The other — the
    earlier retry left running when a later 429 moves the wait out — was argued equivalent here.
    **That argument was unsound and is withdrawn by the fifth round below (ad)**: the retry flushes
    every outbox, the scrobbles among them, which no 429 of these outboxes stops, so a replaced
    retry left running flushes them inside the later wait; a test now kills the mutant. One mutant
    was added after that run: a 429 that does not lengthen the wait left untold, which r6 kills.
    Re-run on the final commit, the added test and mutant included: both killed, and the one
    argued equivalent surviving. By decision, 40 mutants: S1
    three (the settled id left out of the other records, a waiting create keeping it as a
    candidate, mapped ids not excluded), S2 four, S3 two, S4 one, S6 ten, S7 six, S8 two — M13
    killed by r7b and M18 by r7 — r12 five, the origin errors four, r10a three; 39 killed and 1
    surviving (killed in the fifth round). (aa) **Live.** CONF-88..91 against one fresh disposable reference server
    (Navidrome 0.63.2), run twice back-to-back on the JVM: 6 of 6 each time, the server holding
    no playlist before, between or after the runs. Then 6 of 6 on macOS, and 6 of 6 on the JVM
    ...
    **Item 21, fifth review round — a re-review of the fourth round's head found no blocker; the
    ...
    a candidate for another: with two creates of one name in doubt, the person deleted one and kept
    the other, and the kept one adopted the very playlist the person was being offered to delete
    (x1), so confirming the offer deleted the playlist the kept create held (x1b). And a lone
    candidate was adopted while another create of that name was still in doubt, although that
    create's send may have made it (x4). **Decision:** every id an offer names (`PossiblyCreated`,
    `PossibleDuplicate`) joins the pre-send record of every other create of this device when it is
    offered — every other create, not only those of the same name: simpler, and it excludes more —
    and while another create that sent the same name is in doubt, one waiting for the person or
    deleted here included, a lone candidate is named, never adopted. The claim now holds as §18.6
    states it: a playlist this device knows one of its creates made, or has offered for one, is
    never a candidate for another. Its cost is residual 5 of §18.6: a create whose own playlist was
    offered for another is sent again, a duplicate the person can delete. The fourth round's test
    that a create waiting on a choice looks again once another create settles a playlist it named is
    superseded by this rule — that other create is now sent again instead — and replaced by one
    pinning it. Creates of different names in doubt together still each adopt their own lone
    candidate, and a test pins that too. (ac) **x2: a choice another create voided was replaced
    silently.** The person chose one of two candidates; another create then settled on that one, and
    the waiting create adopted the candidate the person had passed over, told only `Created`.
    **Decision:** the voided choice is asked again with the candidates that remain
    (`PossibleDuplicate`), never adopted; with none left the create looks again. (ad) **Finding 2:
    runs of 429s.** A 429 for a change the person had withdrawn while its send was out began a run
    ...
    on the code. (ae) **x3, narrowed.** The fourth round's "nothing is written" to a playlist found by
    inference once its create is deleted here held only for writes not yet started. **Decision:**
    the check runs as late as it can — under the playlist's list lock, just before the comment or
    visibility is written — and the text says what remains: a write already out when the delete
    arrives may land. x3 is pinned as that residual, and a delete made while the create waits for
    the list lock is shown to write nothing. (af) **Finding 3: the wait was checked once per flush.**
    A playlist flush already running when favourites met `Retry-After: 60` sent its next change
    inside the wait (c1), and the reverse held too, so "during the wait neither flush sends
    anything" was false. **Decision:** both flushes check the wait before each change they send.
    (ag) **M8:** the playlist flush's run-ending is pinned as the favourites' is (p8): a flush the
    wait stopped does not end its run. (ah) **The status mapping reaches more than the reader.**
    The mapping of (m) — a bare 401 as `Auth.InvalidCredentials`, any 429 as `Server.Busy` whatever
    its body, any other 4xx or 5xx without an envelope as `Server.HttpStatus` — lives in the one
    checked request path that the library sync, the catalog epoch's reads and the legacy browse
    share with the reader, and its documentation named only the reader. The presentation changed on
    those paths too: where each read a bare error status as a malformed answer, and a 429 with an
    envelope by its generic code, it now reads a non-envelope 4xx or 5xx as a server failure, a 429
    as the server asking to wait, and a bare 401 as refused credentials. (ai) **Wording.** r12 is
    stated as the code behaves (§18.3, §18.6): a change every send of which was answered with a 429,
    another 4xx or another answer proving it unapplied answers `CompactedAway`, and only one whose
    send went out and was not so answered answers `AlreadySent`. The third round's sentences that
    S4 and S6 superseded are marked so, with the rule that replaced them. (aj) **The schema.** The
    columns amended schema 6 in place, on the ground that schema 6 had shipped in no build. But
    schema 6 is on `main` (#139): every developer or CI install built from `main` since holds the
    unamended schema 6 and would never run an amendment. **Decision:** an additive migration.
    ...
    regenerates the committed v7 byte for byte; `databases/7.db` is SQLDelight's own snapshot of
    the `.sq` files, and regenerating it reproduces the committed file byte for byte too. And
    `5.sqm`, `6.db` and the v6 fixture are `main`'s again, byte for byte. The amendment's `CHECK (…
    IN (0, 1))` on `is_public` and `readonly` is gone from both paths: this project's SQLDelight
    dialect refuses a CHECK naming a column that the same `ALTER TABLE` adds (OBSERVED: the
    generator reported "No column found with name is_public"), and a fresh install must be the
    upgraded table exactly; the store writes only 0, 1 or NULL. The migration gate gained
    ...
    **Item 21, seventh review round — a re-check of the sixth round's head found it merge-ready with
    one new NIT; the maintainer made it a fix, with two more before the pull request.** (aw) **A
    create re-sent while an earlier send is in doubt.** A create whose first send was in doubt, sent
    again, edited or deleted here while that re-send was out and answered 429, kept the re-send's
    record: the sixth round's restore required that no earlier send be in doubt. The 429 proves only
    the re-send made nothing; the send still in doubt is the first, which carried another name when
...
20. **Found while implementing R2a-core (the Apple facade over the reader,
    ...
    around it.
21. **Two core gaps found through the facade, since fixed in the core (items 23–24; the first only
    when the epoch read succeeds — item 25 completes it).**
    ...
    them rather than a guess. Search rows carried none either; since item 26 a search's track rows
    ...
    (its probe P4) and fixed by item 25:** a list opened offline and never read, then told
...
23. **Core fix: open searches follow reachability and reconnect (item 21's first gap).**
    ...
    without telling the surfaces. **Corrected in place (item 25): this fix claimed that no search could
    ...
    offline, a later reachability report changed nothing, and the facade reported success. Item 25
    ...
    it drove; item 25's tests drive all three.
...
25. **One reachability model, after independent review of R2a-core (corrects item 23).** The first
    ...
    reconnects on foreground, in either order or only one. A new `reachable` flag holds the platform's
    report. The outbox flush sends while it is true, which is what lets the flush run before the reader
    is online, and a change tapped during a reconnect is sent by it. A failed epoch reading changes
    nothing after the flush: nothing is revalidated or relabelled, the outcome names the error, and
    the next report or reconnect retries. At the transition every open window republishes at once, as
    `revalidating` or `loading` where its read is coming — never `offline` until its turn, never
    `loading` with nothing coming — and a window with nothing cached says `loading` while a read is in
    flight or pending, as a cached one says `revalidating`. The facade's `connect`/`reconnect`
    completions report THIS call's reading and its error kind; `epochKnown` no longer reflects any
    earlier reading. A consequence for §18.3, stated there: a read can overtake only a change made
    while connected, never an offline one. So the three §18.3 conflict tests, which reached the rule by
    refreshing between a reachability report and the reconnect — a sequence the model no longer allows
    — now reach it the way it still happens, through a change whose send failed while connected. One
    more test asserts that an offline change is sent first. OBSERVED 2026-09-24:
    ...
    cancel, the reading that answered late marked the reader online (item 29).
26. **Searches are revalidated like windows, released when closed, and carry playability.** Every
    ...
    none registered. It finds at most one still reachable after collection, and none once the client
    has closed; item 29 says why one is allowed. A closed window subscription, collected, is its
    control. A core test counts 20 listeners registered while open and none left after close.
27. **A viewport is a hint, and never a failure.** An index past the end of the reader's latest
    ...
    test drives the same shrink through `ListWindow`.
28. **Close, cancellation and the review's other findings.** `close()` returns at once, and the
    ...
    the main thread. Seven of the review's surviving mutants (A–G) now have killing tests. They pin
    ...
    with the main-thread check removed (item 29).
29. **Evidence for items 25–28, and what it does not reach.** OBSERVED 2026-09-24 on macOS arm64,
    counted from the JUnit XML with every test task forced to execute. `macosArm64Test` ran 409
    tests, `jvmTest` 370 and `testAndroidHostTest` 474, each with 0 failures. The iOS and macOS
    main sources and the iOS simulator test sources compiled. The Android host suite runs the same
    common code, but no Android shell consumes the session yet.

    **Mutation.** 54 mutants were each compiled and run against a green unmutated baseline: the whole
    JVM suite for core mutants, and the facade class (37 tests) for facade mutants. They cover the
    review's seven survivors (A–G), every rule of items 25–28, and item 22's set carried forward.
    - The first run, of 52, left four alive. Two of those are now killed: F, by the held-thread test
      of item 28, and the removal of the reconnect's cancel on an unreachable report, by the test
      item 25 adds.
    - Two mutants were added later, and both are killed. Each keeps a closed search's change
      listener. The second also disables the test's registry check, which shows the reachability
      check catches the leak on its own.
    - Two survive. One removes the client's close guard, and is equivalent, as in item 22. The other
      lets a throwing viewport publish a failure. It is **ASSUMED** equivalent, from reading the code:
      the reader now clamps every range, so nothing reaches that guard. Reverting all three viewport
      layers at once is killed.

    **A retention that is not the reviewed leak, cause unknown.** The release test first required
    all forty closed search subscriptions to be collected, and it failed intermittently. A
    diagnostic ran its steps 200 times, each on a fresh database. In 13 of those rounds, exactly one
    closed subscription stayed reachable. In every case its listener had been dropped and its core
    search closed. None of the facade's or the core's registries held it, and no coroutine was
    running in either scope. Eight cases were followed further, and each was released only when the
    reader's thread stopped. Three of them survived dropping the composition and cancelling the
    reader scope's work, and one also survived cancelling the main scope. Runs that reused one
    database across rounds found none in 200 rounds. A plain hop between the main thread and a
    single-thread dispatcher retained nothing in 3,200 hops.

    What the test now checks: the session registers none of the forty, at most one is reachable
    after collection, and none is reachable once the client has closed. The leak the review found
    kept all forty. What is ASSUMED: that the one retained object costs nothing that matters. It
    holds the client and its last publication, and is released at close.

    **The Objective-C header** gained exactly two members: `close(completion:)` on the client and
    `playability` on the search row. The rest of its diff is documentation.

    **Not reached.**
    - No Swift compiles against the header yet.
    - iOS is compiled but not run.
    - No test yet runs the public constructor's own composition (item 22).
    - How often a real app hits the close race of item 28 is still ASSUMED.

    R3 continues this record at item 30.
...
1. Run `32599896419`, job `apple-ci`, launched the unentitled app host, delivered the pinned fixture
   URL, and executed the production connector against the real server. The assertions for
   `accountErrorPersistence`, both `credentialPersistenceFailed` representations, absence of the
   active-account pointer, and absence of a stored credential all passed. Only the two subsequent
   offscreen-accessibility string assertions failed.
2. Those passing guards observe the real success callback crossing the Kotlin main dispatcher into
   Swift `@MainActor` and entering the production persistence branch. They do not observe SwiftUI
   rendering: the app-hosted offscreen accessibility tree omitted the expected strings, and the test
   process later exited unexpectedly. The unstable view probe was removed rather than treated as an
   application rendering failure.
3. The renamed control continuously pins the live-derived persistence-failure **state**. The cell
   remains `partial`; actual production persistence, signed-item attributes, the connected UI in the
   live app process, and the live-derived SwiftUI render remain ASSUMED under their named boundaries.
